...
1. 在 Chrome 中安装 **opencli Browser Bridge** 扩展（从 Chrome Web Store 安装或加载 `extension/` 文件夹作为未打包扩展）。
...
- **"Extension not connected" 报错**
  - 确保你当前的 Chrome 已安装且**开启了** opencli Browser Bridge 浏览器扩展。
  - 如果是刚装完扩展，需要重启 Chrome 浏览器。
